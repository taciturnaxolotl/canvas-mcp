...
const routes = {
	...
	"/og.png": Bun.file("src/public/og.png"),
		...
		GET(req: Request) {
			...
			if (detailed) {
				return Response.json({
					...
					cache: {
						apiKeys: DB.getApiKeyCacheStats(),
						canvas: CanvasClient.getCacheStats(),
					},
				});
			}
			...
			return Response.json(
				{
					...
					error_description: "Use Client ID Metadata Documents instead",
				},
				{ status: 501 },
			);
		},
	...
	"/auth/authorize": {
		async GET(req: Request) {
			...
			const code_challenge_method = url.searchParams.get(
			...
			if (code_challenge_method !== "S256") {
				return new Response("Only S256 PKCE method is supported", {
					status: 400,
				});
			}
  ...
  <main>
	...
	"/auth/consent": {
		async POST(req: Request) {
			...
			const code_challenge_method = formData.get(
				"code_challenge_method",
			) as string;
	...
	"/auth/token": {
		async POST(req: Request) {
			...
			const accessToken = DB.createOAuthToken(
				authData.user_id,
				authData.scope,
				86400000,
			);
